...
# =========================================================
# БАЗОВАЯ СХЕМА РЕГЛАМЕНТОВ
# =========================================================
#
# Раньше эти таблицы заводил только разовый скрипт
# scripts/archive_migrations/migrate_regulations.py. На боевой базе он
# когда-то отработал, а на чистой установке сервер падал при запуске:
# init_regulation_extensions сразу шёл делать ALTER TABLE по таблицам,
# которых нет. Теперь схема создаётся при старте и живёт здесь —
# в одном месте с миграциями, которые её дополняют.
#
# Всё через CREATE TABLE IF NOT EXISTS: на работающей базе это пустая
# операция, данные не трогаются.
...
def init_regulation_tables():
    """
    Создаёт таблицы регламентов, если их ещё нет.

    Только таблицы: индексы строит init_regulation_extensions после
    того, как добавит недостающие колонки — часть индексов опирается
    именно на них (например, idx_param_group по param_group).
    """
...
def init_regulation_extensions():
    ...
    # Колонки, добавленные к базовой схеме уже после её появления.
    #
    # Раньше часть из них заводили вручную прямо в базе: в боевой они
    # есть, а в коде их не было нигде. Из-за этого чистая установка
    # получала урезанные таблицы и падала на первом же запросе. Список
    # должен покрывать ВСЁ, чего нет в _BASE_SCHEMA.
    #
    # Порядок важен: сначала таблицы (выше), потом эти колонки, и лишь
    # затем индексы — часть индексов строится по колонкам отсюда.

    later_columns = {
        "regulation_stages": {
            "is_active": "INTEGER NOT NULL DEFAULT 1",
            # связь этапа с его предшественником в прошлой версии
        ...
        "measurements": {
            # норма копируется в замер, чтобы прошлые замеры не
            # пересчитывались по новым границам
