"""
ВНИМАНИЕ: схема отсюда переехала в backend/services/regulation_service.py
(_BASE_SCHEMA, _BASE_INDEXES, init_regulation_tables). Теперь таблицы
создаются при старте сервера, и запускать этот скрипт не нужно.

Файл оставлен как история решения — пояснения ниже объясняют, почему
схема устроена именно так. Править схему здесь бессмысленно: рабочая
копия в regulation_service.py.
